...
async def process_publish_product(callback_query: types.CallbackQuery, bot: Bot):
    ...
    try:
        ...
        if media_group:
            ...
            channel_sent_messages = await bot.send_media_group(
                chat_id=CHANNEL_ID,
                media=[InputMediaPhoto(media=m.media, caption=m.caption, parse_mode=m.parse_mode) for m in media_group] # Копіюємо медіа для каналу
            )
            channel_message_id = channel_sent_messages[0].message_id
            ...
            channel_sent_message = await bot.send_message(
                chat_id=CHANNEL_ID,
                text=caption,
                parse_mode='Markdown'
            )
            channel_message_id = channel_sent_message.message_id
        ...
        if product['moderator_message_id']:
            try:
                ...
                # Також видаляємо попередні повідомлення з фото, якщо вони були
...
async def process_reject_product(callback_query: types.CallbackQuery, bot: Bot):
    ...
    if product['moderator_message_id']:
        try:
            await bot.delete_message(callback_query.message.chat.id, product['moderator_message_id'])
            # Також видаляємо попередні повідомлення з фото, якщо вони були
            photos_file_ids = await get_product_photos_from_db(product_id) # Отримуємо фото для видалення
            if photos_file_ids:
                # Це може бути складно, оскільки media_group надсилає багато повідомлень.
                # Якщо потрібно видалити всі повідомлення групи, потрібно зберегти їх ID.
                # Наразі, просто видаляємо повідомлення з кнопками.
                pass 
        except Exception as e:
            logging.warning(f"Не вдалося видалити повідомлення модератора: {e}")
